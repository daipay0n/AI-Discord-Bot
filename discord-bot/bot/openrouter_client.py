...
def _build_fallback_queue(
    preferred_models: list[str], free_models: list[str]
) -> list[str]:
    preferred_set = set(preferred_models)
    rest = [m for m in free_models if m not in preferred_set]
    random.shuffle(rest)
    return preferred_models + rest
...
async def chat_completion(
    ...
    queue = _build_fallback_queue(preferred_models or [], free_models)
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        for model in queue:
            ...
            try:
                ...
                logger.debug(
                    "HTTP error %d on model %s: %s", e.response.status_code, model, e
                )
                continue
    ...
    logger.error("All %d models exhausted, returning None", len(queue))
    return None
